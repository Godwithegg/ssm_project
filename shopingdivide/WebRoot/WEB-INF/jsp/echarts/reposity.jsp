	...
	<script type="text/javascript">
		...
		var option = {
			...
			tooltip : {
				...
				/*
				控制提示框内容输出格式*/
				formatter : '{b0}<br/><font color=#FF3333>&nbsp;●&nbsp;</font>{a0} : {c0} 件 '
			},
			dataZoom : [ {
		...
		$.ajax({ //使用JQuery内置的Ajax方法
			...
			url : "echarts/showview.action", //请求发送到ShowInfoIndexServlet处
			data : {
				name : "商品种类"  //$(statusCustom)"
			...
			success : function(result) {
				...
				if (result != null && result.length > 0) {
					for (var i = 0; i < result.length; i++) {
						...
						tems.push(result[i].amount); //挨个取出价格、总量、销售量等值并填入前面声明的价格、总量、销售量等数组
					}
